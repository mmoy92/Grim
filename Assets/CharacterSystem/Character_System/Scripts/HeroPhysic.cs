...
public class HeroPhysic : MonoBehaviour
{
	...
	public float hardSoftLandFactor = 5.0f; // Trigger for normal or roll land animation
	...
	void Update ()
	{
		if (useRagdoll) 
		{
			...
			if (isRagdoll)
			{
				...
				if (end || Input.GetKey(hKey.climbOrStandKey)) // For ending manually with "E" or end=true 
				{
					EndRagdoll();
				}
			}
