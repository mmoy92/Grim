...
public class HeroCamera : MonoBehaviour
{
    ...
    void LateUpdate ()
    {
    	...
    	if ( Input.GetKeyDown (hKey.firstPersonCameraKey) ) 
		{
    		// FirstPerson
			lastState = "FirstPerson";
    	
    		cam.camera.fieldOfView = 80.0f;
    		camState = CameraState.FirstPerson;
    	}
		else if ( Input.GetKeyDown (hKey.orbitCameraKey) ) 
		{
    		// Orbit
			lastState = "Orbit";
		...
		else if ( Input.GetKeyDown (hKey.thirdPersonCameraKey) )
		{
    		// ThirdPerson
	...
	void Orbit ()
	{
    	...
    	Quaternion camRot = Quaternion.Euler (yAngl, xAngl, 0);
    	// Camera height
    	Vector3 headPos = new Vector3 (0, -heroHeight /1.5f, 0);
    	// Camera position
    	Vector3 camPos = hero.position - (camRot * Vector3.forward * desDist + headPos);
