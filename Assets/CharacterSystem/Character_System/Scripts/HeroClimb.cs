...
public class HeroClimb : MonoBehaviour 
{
	...
	void Check ()
	{
		...
		if (Input.GetKey(hKey.climbOrStandKey) && (climbState == ClimbState.None || climbState == ClimbState.Edge)
			&& hMove.isAfterTap)
		{
	...
	IEnumerator PreTop ()
	{
		...
		if (Input.GetKey(hKey.climbOrStandKey) && climbState == ClimbState.Prepare) // Climb up
		{
	...
	IEnumerator PreShort (ClimbState cS)
	{
		...
		if (Input.GetKey(hKey.climbOrStandKey) && climbState == ClimbState.PreShort) // Climb up
		{
