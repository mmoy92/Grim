...
public class HeroMotor : MonoBehaviour
{
	...
	void Update ()
	{
		float inputY = Input.GetAxis("Vertical") * (goLeft ? -1 : 1);
		Debug.Log(inputY);
		// MAIN INPUT
		if (canMove && !isDoubleTap) 
		{
			...
			{
				float rotato = Input.GetAxisRaw("Vertical");
				//hero.RotateAround(Vector3.up, rotato);
				if(Input.GetAxisRaw("Vertical")<0){
					goLeft = true;
					hero.eulerAngles = new Vector3(0,270,0);
					
				}
				else{
					goLeft = false;
					hero.eulerAngles = new Vector3(0,90,0);
				}
...
//					Horizontal / Vertical velocity
				...
				{
					// Jump
					if (Input.GetKey (hKey.jumpKey) && canJump)
					{
