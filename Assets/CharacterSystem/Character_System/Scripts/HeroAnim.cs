...
public class HeroAnim : MonoBehaviour 
{
	...
	{
		hero = transform;
		Vector3 localScale = hero.localScale;
	...
	void Movement ()
	{
		...
		if (actionState == ActionState.None && evadeState == EvadeState.None)
		{
			...
			if (Input.GetKeyUp (hKey.forwardKey))
			{
				stopMotion = movement.magnitude > (runSpeed * stopMotionModifier) ? true : false;
			}
			else if (stopMotion && Input.GetAxisRaw("joyRotate") == 0) // stopMotion if not moving sideways
			{
				a.CrossFade ("stop", 0.3f);
				stopMotion = (a["stop"].time) >= 
					(a["stop"].length - 0.01f) ? false : true;
				
				a["stop"].speed = Input.GetKey (hKey.forwardKey) ? 100.0f : 1.0f;
			}
			...
			else if (movement.magnitude < walkSpeed)
			{
				...
				// Turn around 
				{
					...
					// Right
					else if (Input.GetAxis("joyRotate") > 0.0f)
					{
	...
	void UnarmedLogic ()
	{
		...
		// Throw knife
		else if (Input.GetKeyDown (hKey.throwKey))
		{
			actionState = ActionState.ThrowShuriken;
		}
		// Pull lever
		else if (Input.GetKeyDown (hKey.pullLeverKey))
		{
			actionState = ActionState.PullLever;
		}
		// Push button
		else if (Input.GetKeyDown (hKey.pushButtonKey))
		{
			actionState = ActionState.PushButton;
		}
		// Dance 1
		else if (Input.GetKeyDown (hKey.dance1Key))
		{
			actionState = ActionState.Dance1;
		}
		// Dance 2
		else if (Input.GetKeyDown (hKey.dance2Key))
		{
			actionState = ActionState.Dance2;
		}
		// Dance 3
		else if (Input.GetKeyDown (hKey.dance3Key))
		{
	...
	void SwordMovement ()
	{
		...
		if (actionState == ActionState.None && evadeState == EvadeState.None)
		{
			...
			else if (stopMotionSword && Input.GetAxis("Horizontal") == 0) // stopMotion if not moving sideways 
			{
				...
				a["sword_stop"].speed = Input.GetKey(hKey.forwardKey) ? 100.0f : 1.0f;
			}
			// Turning, Moving, Idle etc.
			else if (movement.magnitude < walkSpeed)
			{
	...
	void RifleMovement ()
	{
		...
		if (aC != null)
		{
			...
			// "R" for Reload
			else if (Input.GetKeyDown (hKey.reloadKey))
			{
